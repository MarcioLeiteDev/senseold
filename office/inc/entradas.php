<div class="container-fluid">
    ...
    <div class="col-12">
        <h3 class="bg bg-light p-2 text-gray">Lançar Entradas</h3>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">entradass</h5>
...
if (!empty($search["inicio"])) {
    ...
    $read->ExeRead("invoices", "WHERE mode = :mod AND date_payment BETWEEN :D1 AND :D2 AND status = :s  LIMIT :limit OFFSET :offset", "D1={$inicio}&D2={$fim}&mod=1&s={$search["status"]}&limit={$pager->getLimit()}&offset={$pager->getOffset()}");
    $read->getResult();
} else {
    ...
    $read = new Source\Models\Read();
    $read->ExeRead("invoices", "WHERE mode = :mod AND MONTH(date_payment) = :m AND YEAR(date_payment) = :y ORDER BY date_payment ASC LIMIT :limit OFFSET :offset", "m={$mes}&y={$ano}&mod=1&limit={$pager->getLimit()}&offset={$pager->getOffset()}");
    $read->getResult();
}
